...
const MULTIPLY_DIVIDE_REGEX =
  /(?<operand1>\S+)\s*(?<operation>[\/\*])\s*(?<operand3>\S+)/;

const ADD_SUBTRACT_REGEX =
  /(?<operand1>\S+)\s*(?<operation>(?<!e)[\-\+])\s*(?<operand3>\S+)/;

const EXPONENT_REGEX = /(?<operand1>\S+)\s*(?<operation>\^)\s*(?<operand3>\S+)/;
...
function handleMath({ operand1, operation, operand3 }) {
  const number1 = parseFloat(operand1);
  const number2 = parseFloat(operand3);
